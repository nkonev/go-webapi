...
func TestRegisterResend(t *testing.T) {
	...
	runTest(container, func (e *echo.Echo){
		...
		c2, _, hm2 := request("POST", "/auth/register/resend-confirmation-token", strings.NewReader(`{"email": "newroot@yandex.ru"}`), e, "")
		assert.Equal(t, http.StatusOK, c2)
		...
		emailBody := m.Calls[1].Arguments[2].(string)
		assert.Contains(t, emailBody, "Go to link for complete your registration http://localhost:1234/confirm/registration?token=")

		emailBody1 := m.Calls[2].Arguments[2].(string)
		assert.Contains(t, emailBody1, "Go to link for complete your registration http://localhost:1234/confirm/registration?token=")

		assert.Equal(t, emailBody, emailBody1)
